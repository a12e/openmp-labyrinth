...
int main(int argc, char **argv)
{
	...
	/* initialise l : murs autour, vide à l'intérieur */
	for( i=0 ; i<N ; i++ )
		for( j=0 ; j<M ; j++ )
	...
	/* place <nbilots> ilots aléatoirement a l'interieur du laby */
	for( ; nbilots ; nbilots-- )
	{
		i = rand()%(N-4) + 2;
		j = rand()%(M-4) + 2;
	...
	/* affiche les cases constructibles */
	newcolor(0,1,1);
	for( i=1 ; i<N-1 ; i++ )
		for( j=1 ; j<M-1 ; j++ )
			if( l[i][j] == -1 )
...
#ifdef AFFICHE
	refresh();
	waitgraph(); /* attend un clic de l'utilisateur */
	closegraph();
#endif /* AFFICHE */
